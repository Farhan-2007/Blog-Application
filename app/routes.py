...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        flash(f'Congratulation Your Account Has Created! Now You Can Login as {form.username.data}!', 'success')
...
def login():
    ...
    if form.validate_on_submit():
            ...
            if user and bcrypt.check_password_hash(user.password, form.password.data):
                ...
                flash('Unsuccessful Login. Please Check Your Email or Password', 'danger')
...
def send_reset_email(user):
     ...
     msg = Message('Password Reset Request', sender = 'sfarhan6388@gmail.com',recipients = [user.email])
...
def reset_request():
     ...
     if form.validate_on_submit():
          ...
          flash('An email has sent with instruction to reset pyour password.')
          return redirect(url_for('login'))
